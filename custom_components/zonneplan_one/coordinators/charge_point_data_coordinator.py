...
class ChargePointDataUpdateCoordinator(ZonneplanDataUpdateCoordinator):
    ...
    def stage_dynamic_charge_value(self, path: str, value: Any) -> None:
        ...
        if self._dynamic_charge_values_equal(path, value, api_value):
            ...
            unit = DYNAMIC_CHARGE_AMOUNT_UNITS.get(path)
            if unit:
                self._last_edited_dynamic_charge_unit = unit
    ...
    def _build_dynamic_charge_params(self, *, warn: bool) -> dict | None:
    ...
    def dynamic_charge_params_are_valid(self) -> bool:
        return self._build_dynamic_charge_params(warn=False) is not None

    async def async_apply_dynamic_charge(self) -> None:
        params = self._build_dynamic_charge_params(warn=True)
        if params is None:
            return
